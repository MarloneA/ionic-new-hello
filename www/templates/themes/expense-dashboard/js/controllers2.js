appControllers.controller('revenueReportCtrl', function(APIBASE, $firebaseArray, $firebaseObject, $scope, $rootScope, $timeout, $http, $filter, $q, $interval, $state, $stateParams, $ionicHistory, $ionicScrollDelegate, $location) {
    ...
    var bestPrices = [];
    var bestLabels = [];
    var noOfBestSellers = 7;
    $scope.noOfBestSellers = noOfBestSellers;
    //$timeout(function(){
    // Add comma to output
    //-------------------------------------------------------------------//
   ...
   //Adjust page chart height
    ...
    $scope.weeksBarCharts = function(){
        ...
        $timeout(function(){
            console.log('8888');
            $location.hash('rev-active-tab-pill-button');
    ...
    FBSalesref.orderByKey().on("value", function(snapshot) {
        ...
        for (var i = 0; i < dailySales.length; i++) {
            ...
            for (var k = 0; k < theDayArray.length; k++) {
                ...
                salesChartData.push({
                    'day':theDayKey,
                    'itemSellPrice':theDayArray[k]['itemSellPrice'],
                        ...
                        Chart.helpers.each(Chart.instances, function(chart) {
                            ...
                            chart.data.datasets.forEach(function(dataset) {
                                if(theChartLabel === 'WeeklyInit'){
                                    ...
                                    // tempDataLabels = tempDataLabels.filter(function(item, pos) {
                                    //     return tempDataLabels.indexOf(item) == pos;
                                    // })

                                    // tempDataset = tempDataset.filter(function(item, pos) {
                                    //     return tempDataset.indexOf(item) == pos;
                                    // })

                                    chart.config.data.labels = tempDataLabels.slice(0, noOfBestSellers);//["1: " + bestOne + "", "2: " + bestTwo + ""];
                                    dataset.data = tempDataset.slice(0, noOfBestSellers);
                    ...
                    */
                    ...
                    new Chart('myChartRev', {
                          ...
                          data: {
                              ...
                              labels: tempDataLabels.slice(0, noOfBestSellers),//["1: " + bestOne + "", "2: " + bestTwo + ""],
                              ...
                              datasets: [{
                                  ...
                                  data: tempDataset.slice(0, noOfBestSellers),//[bestPriceOne, bestPriceTwo],
        ...
        FBSalesref.orderByKey().on("value", function(snapshot) {
            ...
            console.log('currentWkChartcurrentWkChartcurrentWkChartcurrentWkChartcurrentWkChart');
            $timeout(function() {
                var showWksTabChart = setInterval(function(){
                    var showWksTab = $('#dashboard-content md-card .md-actions.weeks a');
                    console.log("GoTabsA: "+showWksTab.length);
                    ...
                    if(showWksTab.length>0){
                        ...
                        (function($){
                            ...
                            setTimeout(function() {
                                ...
                                $('.the-revenue-report.the_purchases [data-toggle-box]').unbind('click');

                                $('.the-revenue-report.the_purchases [data-toggle-box]').on('click', function() {
                                    ...
                                    if(exeCount<1){
                                        if ($('.the-revenue-report.the_purchases #' + toggle_box).is(":visible")) {
                                            $('.the-revenue-report.the_purchases #' + toggle_box).slideUp(250);
                                            console.log('isvisible');
                                        } else {
                                            $(".the-revenue-report.the_purchases [id^='box']").slideUp(250);
                                            $('.the-revenue-report.the_purchases #' + toggle_box).slideDown(250);
        ...
        $scope.monthlyBarCharts = function() {
            ...
            $timeout(function(){
                console.log('8888');
                $location.hash('rev-active-tab-pill-button-month');
                $ionicScrollDelegate.anchorScroll(true);
        ...
        $scope.theYearlyBarCharts = function(fromYear,toYear) {
            ...
            var currentYearBarCharts = function(fromYear,toYear) {
                ...
                //Merge all similar sales by itemIDs
                    ...
                    console.log('compiledSaYYYYYYYYales');
                    console.log(compiledSales);
        ...
        $scope.yearlyBarCharts = function() {
            ...
            $timeout(function(){
                console.log('8888');
                $location.hash('rev-active-tab-pill-button-year');
                $ionicScrollDelegate.anchorScroll(true);
