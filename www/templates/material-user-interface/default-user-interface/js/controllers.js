...
appControllers.controller('defaultUserInterfaceCtrl', function(APIBASE, APIBASE_V1, $mdDialog, $mdBottomSheet, $mdToast, $mdDialog, $firebaseArray, $firebaseObject, $scope, $rootScope, $timeout, $http, $filter, $q, $interval, $state, $stateParams, $ionicHistory, FileUploader) {
   ...
   //CUSTOM CODE
        ...
        var FBSalesref = firebase.database().ref().child('puchases'+company_id);
        ...
        $http({
            url: APIBASE+'get_items?company_id='+company_id,
            method: "GET",
            //data: purchaseParams,
            headers: {
            ...
            var portalDataProds = resp.data;
                ...
                FBProdsref.orderByKey().on("value", function(snapshot) {
                    ...
                    if(saleItems === null){
                        portalDataProds.forEach(function(item) {
                        ...
                        portalDataProds.forEach(function(item) {
                            ...
                            for (var key in saleItems) {
                                if (saleItems.hasOwnProperty(key)) {
                                    console.log(key + " -=-=> " + saleItems[key]);

                                    var saleItems2 = saleItems[key];

                                    for (var key2 in saleItems2) {
                                        if (saleItems2.hasOwnProperty(key2)) {
                                            console.log(key2 + " -=-=!> " + saleItems2[key2]);
                                            if(key2 === 'item_code'){
                                                if(prodCode==saleItems2[key2]){
                                                    prodExists = true;
                                                }
                                            }
                                        }
                ...
                // var newStockItem = {
                //     "id": saleItems[saleItems.length-1]['id'],
                //     "alias": null,
                //     "item_code": saleItems[saleItems.length-1]['item_code'],
                //     "item_type_id": saleItems[saleItems.length-1]['item_type_id'],
                //     "title": saleItems[saleItems.length-1]['title'],
                //     "description": "",
                //     "cost_price": saleItems[saleItems.length-1]['cost_price'],
                //     "sell_price": saleItems[saleItems.length-1]['sell_price'],
                //     "in_stock": 0,
                //     "photo_1": null,
                //     "photo_2": null,
                //     "photo_3": null
        ...
        FBProdsref.orderByKey().on("value", function(snapshot) {
            ...
            $scope.sellItems = function() {
                ...
                $scope.items.forEach(function(item) {
                    if (item.selected === true) {
                        ...
                        theSale = {
                            ...
                            'quantity': Number(item.count),
                            'itemSellPrice': Number(item.cost_price) * Number(item.count),
                            'ts': theTimeStamp
                        }
                        ...
                        checkedItems.push({
                            "id": (item.id).toString(),
                            "amount": (item.cost_price).toString(),
                ...
                var purDate = d.getDate();
                var purMonth = d.getMonth() + 1;
                var purYear = d.getFullYear();
                ...
                var theDate = purYear + "-" + purMonth + "-" + purDate;
                var theTimeStamp = Date.UTC(d.getFullYear(), d.getMonth(), d.getDate(), d.getHours(), d.getMinutes(), d.getSeconds())-(1000*60*60*3); //add month -(minus) 1
                ...
                var expensesRef = $firebaseArray(theDayExpesesRef);
                var thePurchaseExpense = {
                    'time': theTime,
                    //'itemTitle': $scope.expenseName,
                    'itemID': 1,
                    'itemType': 'COGS',
                    'itemDesc': 'New Stock',
                    'imgUrl':'http://www.grass2grace.co.ke/pwc_app/uploads/unnamed.png',
                    'itemCost': Number(totalSaleAmount),
                    'ts': theTimeStamp
                }
                expensesRef.$add(thePurchaseExpense);
                ...
                $scope.confirmCheckOut = function() {
                    ...
                    setTimeout(function() {
                        $http({
                            url: APIBASE_V1 + 'post_purchase?company_id='+company_id,
                            method: "POST",
                            data: postSaleParams,
                            headers: {
                                'Content-Type': 'application/x-www-form-urlencoded'
                ...
                //FBProdsref.orderByKey().on("value", function(snapshot) {
                    ...
                    $scope.sellItems = function() {
                        ...
                        $scope.confirmCheckOut = function() {
                            ...
                            setTimeout(function() {
                                $http({
                                    url: APIBASE+'post_purchase?company_id='+company_id,
                                    method: "POST",
                                    data: postSaleParams,
                                    headers: {
                                        'Content-Type': 'application/x-www-form-urlencoded'
        ...
        $scope.showConfirmDialog = function($event) {
            $mdDialog.show({
                ...
                template:
                    ...
                    '<md-dialog md-theme="mytheme">' +
                    '  <md-dialog-content>'+
                    '<div class="confirmsale bottom-0"><h1 class="uppercase ultrabold bottom-0">CONFIRM PURCHASE</h1><h6 class="uppercase ultrabold bottom-0">ENTER CASH RECEIVED</h6><div class="page-login-field"><input ng-model="parent.cashReceived" type="text" placeholder="KES"> <em>(required)</em></div><h6 class="uppercase ultrabold bottom-0">TOTAL: KES {{parent.totalSaleAmount}}</h6><h6 class="uppercase ultrabold bottom-0">CHANGE: KES {{(parent.cashReceived-parent.totalSaleAmount > 0) ? parent.cashReceived-parent.totalSaleAmount:"0";}}</h6><div class="sale_item_list" style=""><table class="table-borders-light" width="100%"><tbody width="100%"><tr width="100%"><th class="bg-night-dark" width="50%">Items</th><th class="bg-night-dark" width="20%">Price</th><th class="bg-night-dark" width="10%">Qty</th><th class="bg-night-dark" width="20%">Total</th></tr><tr ng-repeat="item in parent.clientReceipt"><td class="bg-night-light">{{item.title}}</td><td class="bg-night-light">{{item.amount}}</td><td class="bg-night-light">{{item.quantity}}</td><td class="bg-night-light">{{item.quantity*item.amount}}</td></tr></tbody></table></div><a href="#" ng-click="parent.confirmCheckOut(); cancel();" class="md-raised md-button md-default-theme">CONFIRM</a></div>'
                      + '  </md-dialog-content>' +
                      '</md-dialog>',
                ...
                $timeout(function() {
                    $mdDialog.show({
                        ...
                        locals: {
                            displayOption: {
                                title: "Good!",
                                content: "You have made a successfully purchase",
                                ok: "Confirm"
                            }
                        }
                    }).then(function () {
        ...
        $scope.addStockItemsDialog = function($event) {
            ...
            //$scope.showSaleAlertDialog = function ($event) {
                //$timeout(function() {
                    ...
                    $http({
                        url: APIBASE+'post_item?company_id='+company_id,
                        method: "POST",
                        ...
                        $http({
                            ...
                            //FBProdsref.orderByKey().on("value", function(snapshot) {
                                ...
                                //Also copy the added stock item to Firebase DB
                                ...
                                saleItems.forEach(function(item) {
                                    ...
                                    //FBProdsref.orderByChild("id").equalTo(item['id']).on("child_added", function(snapshot) {

                                        //var returnData = snapshot.val();

                                        console.log('thereturnDatathereturnData');
                                        ...
                                        //     //FBProdsref.child(theKey).remove();

                                        //     // FBProdsref.child(theKey).update({
                                        //     //     latestAmountPaid: totalLoanPaid,
